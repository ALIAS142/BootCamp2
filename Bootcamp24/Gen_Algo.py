...
# '''
# Сделаем Class Chromosome, у которого будет три свойства:
...
# '''
# Теперь напишем функцию для генерации случайной хромосомы.Функция принимает два парметра:
# длина хромосомы, которую нужно получить и набор генов, из которых нужно сдлеать хромосому.
# '''
...
# '''
# В функцию заполнения популяции мы также передаем размер популяции, размер хромосомы(длина строки) и генофонд(все буквы английского алфавита),
# чтобы не зависеть от глобальных переменных.
# Подбор пар:
# '''
...
# котрая заполняет вторую половину популяции потомками (родители сохраняются в первой популяции). 
# '''
...
# '''
# Среди выживших нужно отобрать пары родителей, для того, чтобы получить от них потомков
...
# Здесь также есть куча методов, и мы опять возьмем то, что проще.
# Это будет одноточечный кроссинговер.
# Мы выбираем случайную позицию внутри хромосомы, и потомок получает гены родитедя №1 от начала и до этой позиции,
# и гены родителя №2 от этой позиции и до конца.
# Каждые два родителя порождают пару потомков. То есть функцию cross() вызываем дважды (Сначала родитель1, родитель2, затем наоборот).
# '''
...
# Были нередки случаи, когда строка уже почти правильная, то есть отличалась только одним символом, 
# но если после мутации в ней меняется больше, чем один символ, мы наоборот удаляемся от цели.
# '''
...
# '''
# Генофонд - это строка-справочник, которая содержит все возможные гены. Его и финальную строку (хромосому)
# закодируем в байтовые массивы.
# '''
gene_pool = bytearray(b'abcdefghijklmnopqrstuvwxyz ') #английский алфавит и пробел
...
# ТО есть, поместим наших избранников в список "выживших".
...
# Мы также печатаем текущую популяцию на каждом шаге циклаю
# '''